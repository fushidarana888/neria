(()=>{
  ...
  function eachStorage(fn){
    [localStorage,sessionStorage].forEach(storage=>{
      try{fn(storage)}catch{}
    });
  }

  function removeAllDemoStates(){
    eachStorage(storage=>{
      Object.keys(storage).forEach(key=>{
        if(key.startsWith(STATE_PREFIX)||key==='neria_demo_state'||key.startsWith('neria_demo_launch_')) storage.removeItem(key);
      });
    });
  }

  function removeLegacyStates(){
    eachStorage(storage=>{
      Object.keys(storage).forEach(key=>{
        if((key.startsWith(STATE_PREFIX)||key==='neria_demo_state')&&key!==KEY) storage.removeItem(key);
        if(key.startsWith('neria_demo_launch_')&&key!==LAUNCH_KEY) storage.removeItem(key);
      });
    });
  }

  function candidateTime(s){
    let t=Number(s?.startedAt)||0;
    if(Array.isArray(s?.timeline)) for(const x of s.timeline)t=Math.max(t,Number(x?.at)||0);
    if(Array.isArray(s?.messages)) for(const x of s.messages)t=Math.max(t,Number(x?.at)||0);
    if(Array.isArray(s?.transfers)) for(const x of s.transfers)t=Math.max(t,Number(x?.at)||0);
    return t;
  }
  ...
  function readLegacyIdentity(){
    ...
    eachStorage(storage=>{
      Object.keys(storage).forEach(key=>{
        if(!(key.startsWith(STATE_PREFIX)||key==='neria_demo_state'))return;
        try{
          const s=JSON.parse(storage.getItem(key)||'null');
          if(!s||typeof s!=='object')return;
          const nickname=String(s.nickname||'').trim();
          const role=VALID_ROLES.has(String(s.role||''))?String(s.role):'';
          candidates.push({nickname,role,time:candidateTime(s),session:storage===sessionStorage});
        }catch{}
      });
    });
    candidates.sort((a,b)=>{
      const usefulA=(a.nickname&&a.nickname!=='Будущий гражданин'?2:0)+(a.role?1:0);
      const usefulB=(b.nickname&&b.nickname!=='Будущий гражданин'?2:0)+(b.role?1:0);
      return usefulB-usefulA||b.time-a.time||Number(b.session)-Number(a.session);
    });
    const c=candidates[0];
    return c?{nickname:c.nickname||'Будущий гражданин',role:c.role||''}:null;
  }

  function cameFromLauncher(){
    try{
      const here=location.pathname.split('/').pop();
      if(here!=='demo-path.html')return false;
      if(!document.referrer)return false;
      const u=new URL(document.referrer);
      const refPage=u.pathname.split('/').pop();
      return refPage===''||refPage==='index.html';
    }catch{return false}
  }

  if(cameFromLauncher()){
    const identity=readLegacyIdentity();
    removeAllDemoStates();
    if(identity)sessionStorage.setItem(LAUNCH_KEY,JSON.stringify(identity));
  }else{
    removeLegacyStates();
  }
  ...
  function fresh(seed={}){
    ...
    return{
      version:6,
      runId:newRunId(),
      nickname:String(seed.nickname||'Будущий гражданин').slice(0,30),
      role:VALID_ROLES.has(String(seed.role||''))?String(seed.role):'',
      balance:100,
      startedAt:now,
      timeline:[{id:'start',at:now,icon:'🏛️',title:'Первый день в Нерии',description:'Демо-гражданство получено. Настоящий гражданский ID появится только после регистрации.'}],
      achievements:[],visited:[],messages:[],transfers:[],vote:null,dealDone:false,articleRead:false
    };
  }
  ...
  function normalize(raw){
    const base=fresh({nickname:raw?.nickname,role:raw?.role});
    const s=Object.assign(base,raw||{});
    s.version=6;
    s.nickname=String(s.nickname||'Будущий гражданин').slice(0,30);
    s.role=VALID_ROLES.has(String(s.role||''))?String(s.role):'';
    s.balance=Number.isFinite(Number(s.balance))?Number(s.balance):100;
    s.timeline=Array.isArray(s.timeline)?s.timeline:base.timeline;
  ...
  function load(){
    ...
    const s=fresh(consumeLaunch()||{});
    return save(s);
  }

  function start(nick,role){
    removeAllDemoStates();
    const s=fresh({nickname:nick,role});
    return save(s);
  }
