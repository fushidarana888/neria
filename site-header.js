(()=>{
  ...
  function addStyle(href,key){if(document.querySelector('link[data-'+key+']'))return;const l=document.createElement('link');l.rel='stylesheet';l.href=href;l.setAttribute('data-'+key,'1');document.head.appendChild(l)}
  ...
  function init(){
    addStyle('site-polish.css?v=1','neria-polish');
    addScript('site-polish.js?v=1','neria-polish');
    ...
    const setOpen=open=>{panel.hidden=!open;toggle.setAttribute('aria-expanded',open?'true':'false');toggle.classList.toggle('open',open);document.documentElement.classList.toggle('neria-menu-open',open&&matchMedia('(max-width:820px)').matches)};
    toggle.addEventListener('click',e=>{e.stopPropagation();setOpen(panel.hidden)});
    panel.addEventListener('click',e=>e.stopPropagation());
    document.addEventListener('click',()=>setOpen(false));
    document.addEventListener('keydown',e=>{if(e.key==='Escape'&&!panel.hidden){setOpen(false);toggle.focus()}});window.addEventListener('resize',()=>{if(innerWidth>820&&!panel.hidden)document.documentElement.classList.remove('neria-menu-open')});
